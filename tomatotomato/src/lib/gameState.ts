/**
 * In-memory game state manager for Apples to Apples style game
...
class GameStateManager {
  ...
  private gameAnswers: Map<string, { [promptId: string]: { [playerId: string]: string } }> = new Map();
  ...
  deleteGame(gameId: string): void {
    ...
    this.gameAnswers.delete(gameId);
  }
  ...
  recordAnswer(gameId: string, promptId: string, playerId: string, answer: string): void {
    if (!this.gameAnswers.has(gameId)) {
      this.gameAnswers.set(gameId, {});
    }
    
    const answers = this.gameAnswers.get(gameId)!;
    if (!answers[promptId]) {
      answers[promptId] = {};
    }
    
    answers[promptId][playerId] = answer;
  }

  getAnswers(gameId: string): { [promptId: string]: { [playerId: string]: string } } {
    return this.gameAnswers.get(gameId) || {};
  }

  clearAnswers(gameId: string): void {
    this.gameAnswers.delete(gameId);
  }
